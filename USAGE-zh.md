...
当包发布到 npm 后，用户可以直接全局安装，并在任意终端目录启动：
...
- 当前终端所在目录会被当作工作区
- `actov --workspace <path>` 可以显式指定工作区
- 状态、历史记录和本地配置默认都相对这个工作区生效，除非你在配置里另行修改
...
当前仓库已经加入 GitHub Actions 的 npm 发布工作流：
...
1. 先在 npmjs.com 上为这个包配置 trusted publishing
2. 填这些值：
   - GitHub 用户或组织：`DeconBear`
   - 仓库名：`actoviq_claw`
   - workflow 文件名：`npm-publish.yml`
3. 修改 `package.json` 里的版本号
4. 推送一个与版本匹配的 Git tag，例如 `v0.1.0`
...
独立的 `CI` 工作流会在普通 push 和 pull request 时自动运行，这样即使不发布 npm，也会先完成代码健康检查。
